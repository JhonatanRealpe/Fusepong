...
public class UsuarioRepository implements IUsuario {
	...
	public Optional<Usuario> findUsuario(String email, String pass) {
		return Optional.of(jdbcTemplate.queryForObject("select u.*, e.nombre as nombreempresa from bdfusepong.usuario u LEFT JOIN bdfusepong.empresa e ON e.idempresa = u.idempresa where u.email = ? and u.contrasena = ? ",
				new Object[] { email, pass }, new BeanPropertyRowMapper<Usuario>(Usuario.class)));
	}
